...
def grouped_conn_mat(conn_matrices: list, network: str = 'yeo7', statistic: str = 'mean'):
    ...
    Args:
        conn_matrices: list of connectvity Matrices
        network: Yeo7 or Yeo17 network
        statistic: Summary statistic to be applied (mean, min or max)
    ...
    for conn_mat in conn_matrices:
        ...
        mat = pd.DataFrame(np.nan_to_num(conn_mat))

        if network == 'yeo7':
            yeo = 8
            grpd_mat = np.zeros((8, 8))
        elif network == 'yeo17':
            yeo = 18
            grpd_mat = np.zeros((18, 18))

        for i in range(yeo):
            for j in range(yeo):
                if network == 'yeo7':
                    rows = [int(x) - 1 for x in lab_to_yeo7.keys() if lab_to_yeo7[x] == str(i)]
                    cols = [int(x) - 1 for x in lab_to_yeo7.keys() if lab_to_yeo7[x] == str(j)]
                elif network == 'yeo17':
                    rows = [int(x) - 1 for x in lab_to_yeo17.keys() if lab_to_yeo17[x] == str(i)]
                    cols = [int(x) - 1 for x in lab_to_yeo17.keys() if lab_to_yeo17[x] == str(j)]

                if statistic == 'mean':
                    grpd_mat[i, j] = mat.iloc[rows, cols].mean().mean()
                elif statistic == 'min':
